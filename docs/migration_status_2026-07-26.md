...
- 默认网关：`192.168.31.1`
- 路由器 HTTP：可访问
- 机械臂 `192.168.31.60`：检查时曾 ping 无响应、HTTP 超时、邻居表为 `FAILED`；后续确认原因是急停按钮未复位，复位后浏览器访问恢复正常。

当前主机已在 `192.168.31.0/24` 局域网内。若后续再出现机械臂不可达，除了检查电源、网线、路由器 LAN 口和静态地址保留，也要先确认急停按钮已复位。
