...
Small wrist-yaw corrections can make the vendor IK jump to another wrist branch. `grasp/min_align_yaw_delta_deg` skips automatic gripper-axis alignment below that threshold; the gripper remains close enough for the block while the descent stays on a more continuous IK branch.
