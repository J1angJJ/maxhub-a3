...
During planning, `block_grasp.py` also predicts the physical cuboid's eight 3D corners from the known block dimensions, the table plane, and the estimated long/short axes. The log field `predicted physical cuboid corners` contains `top_0..top_3` and `bottom_0..bottom_3` with base-frame coordinates and projected image pixels. These are model-predicted cuboid corners, including hidden lower corners, not just extra 2D rectangle samples.
