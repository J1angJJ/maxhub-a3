# maxhub-a3 Foxy Docker

本文档记录 MAXHUB A3 项目的 ROS 2 Foxy 容器运行配置。当前不构建项目专属镜像，只复用共享 Foxy 镜像：

```text
ubuntu-env:foxy-source -> ubuntu-env:foxy-user
```

共享镜像由 `/home/j1angjj/workspace/ubuntu-env/docker/foxy` 维护；本目录只放本项目的 compose、设备映射、SDK/参考仓库挂载和本机配置模板。

## Files

```text
workspace/foxy/docker/maxhub-a3-foxy/
├── compose.yaml
├── compose.camera.yaml
├── .env.example
└── README.md
```

## Prerequisites

确认共享 Foxy 镜像已经存在：

```bash
docker image inspect ubuntu-env:foxy-source
docker image inspect ubuntu-env:foxy-user
```

如果缺少 `ubuntu-env:foxy-source`，先手动准备源镜像：

```bash
docker pull osrf/ros:foxy-desktop
docker tag osrf/ros:foxy-desktop ubuntu-env:foxy-source
```

如果缺少 `ubuntu-env:foxy-user`，先构建共享环境：

```bash
cd /home/j1angjj/workspace/ubuntu-env/docker/foxy
./scripts/build.sh
```

## Local Env

复制本项目 env 文件：

```bash
cd /home/j1angjj/workspace/maxhub-a3/workspace/foxy/docker/maxhub-a3-foxy
cp .env.example .env
```

当前默认约定：

- 宿主 ROS 2 工作区：`/home/j1angjj/workspace/maxhub-a3/workspace/foxy`
- 容器主挂载点：`/workspace`
- 现有 CArm SDK：只读挂载到 `/opt/maxhub-a3/arm_control_sdk`
- 官方 demo 参考仓库：只读挂载到 `/opt/maxhub-a3/reference/carm_demo`
- 机械臂 IP：`192.168.31.60`
- 相机映射：默认关闭，确认 `/dev/video*` 和 `/dev/v4l/by-id` 后再叠加 `compose.camera.yaml`

`.env` 可以按当前机器情况调整，例如 `DISPLAY`、`VIDEO_DEVICE`、`ROS_DOMAIN_ID`。相机和串口设备暂不默认映射，后续确实需要时再加实验 overlay。

## Validate Config

只展开 compose 配置，不启动容器：

```bash
cd /home/j1angjj/workspace/maxhub-a3/workspace/foxy/docker/maxhub-a3-foxy
docker compose config
```

应确认：

- `image: ubuntu-env:foxy-user`
- `network_mode: host`
- `/home/j1angjj/workspace/maxhub-a3/workspace/foxy` 挂载到 `/workspace`
- CArm SDK 和官方 demo 为只读挂载
- 默认不映射相机；叠加 `compose.camera.yaml` 后再确认 `/dev/video*` 和 `/dev/v4l`

## Run Shell

进入一次性容器：

```bash
cd /home/j1angjj/workspace/maxhub-a3/workspace/foxy/docker/maxhub-a3-foxy
docker compose run --rm maxhub-a3-foxy bash
```

确认相机已经连接后，叠加相机配置：

```bash
docker compose -f compose.yaml -f compose.camera.yaml run --rm maxhub-a3-foxy bash
```

检查 ROS 2：

```bash
docker compose run --rm maxhub-a3-foxy ros2 --help
```

检查 SDK 挂载：

```bash
docker compose run --rm maxhub-a3-foxy \
  bash -lc 'test -f /opt/maxhub-a3/arm_control_sdk/setup.bash && ls /opt/maxhub-a3/reference/carm_demo/carm_ros2/src'
```

## Workspace

新 ROS 2 包放在：

```text
/home/j1angjj/workspace/maxhub-a3/workspace/foxy/src
```

编译 colcon 工作区：

```bash
docker compose run --rm maxhub-a3-foxy \
  bash -lc 'cd /workspace && colcon build --symlink-install'
```

如果需要编译官方 `carm_ros2` 参考包，建议先复制到 `/workspace/src` 再改，保留 `/opt/maxhub-a3/reference/carm_demo` 只读。

## Boundaries

这里先只承载 ROS 2/Foxy 的项目运行配置。强化学习训练栈、PyTorch、仿真器、数据集或特定实验依赖后续单独增加项目镜像或实验 overlay，不放进共享 Foxy 镜像。
