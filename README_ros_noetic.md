...
注意：`gripper_tcp` 的 `0.149 m` 偏移来自模型几何边界的初始估计，不等价于真实夹持接触点。正式把抓取规划从 `flange` 改成 `gripper_tcp` 前，需要用尺量或低速轻触桌面标定真实 TCP/contact height。当前下探抓取仍应保持 `grasp_z_m` 保守，并优先用 plan 检查。
