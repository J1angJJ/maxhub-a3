...
脚本会先调用 IK；`execute` 开始运动前会默认把夹爪收至 `gripper/overview_pos_m`，减少观测画面遮挡。整段初始化由 `max_total_joint_delta_rad` 限制，执行时再按 `segment_delta_rad` 自动拆成多段 `move_joint`，避免一次命令超过 motion service 的单步关节差值限制。
