...
脚本会先调用 IK；`execute` 开始运动前会默认把夹爪收至 `gripper/overview_pos_m`，减少观测画面遮挡。当前默认值为 `0.005 m`，如果实际表现仍是打开，需要用 `motion_cli.py set-gripper` 标定夹爪间距语义。整段初始化由 `max_total_joint_delta_rad` 限制，执行时再按 `segment_delta_rad` 自动拆成多段 `move_joint`，避免一次命令超过 motion service 的单步关节差值限制。
...
- `set_gripper(pos, tau)`：设置两指间隔和夹持力，文档标注范围为 `pos: 0-80 mm`、`tau: 0-100 N`。
- `get_gripper_state()`：返回 `-1` 未连接/无夹爪、`0` 未使能、`1` 正常、`>1` 伺服错误。
- `get_gripper_pos()`、`get_gripper_vel()`、`get_gripper_tau()`：读取实际夹爪位置、速度和力矩。
- `get_plan_gripper_pos()`、`get_plan_gripper_tau()`：读取规划位置和规划力矩。
- `track_joint()`、`track_pose()`、`move_joint_traj()`、`move_pose_traj()` 也带有可选夹爪位置参数。
...
```bash
rosrun carm_a3_motion motion_cli.py extended
rosrun carm_a3_motion motion_cli.py set-gripper 0.005 --tau 5
sleep 1
rosrun carm_a3_motion motion_cli.py extended
rosrun carm_a3_motion motion_cli.py set-gripper 0.065 --tau 5
sleep 1
rosrun carm_a3_motion motion_cli.py extended
```
...
### 2026-07-20 Grasp Descent Blocked After Table Contact

第一版颜色块抓取执行测试中，规划的抓取位姿相对实际桌面过低，机械臂触碰桌面后未自动停止。已通过实体急停结束任务并归位。

处理结论：

- 在夹爪 TCP、桌面高度和接触停止策略标定前，禁止默认执行 grasp/lift 下探段。
- `block_grasp.py execute` 改为默认只移动到 approach 位姿。
- 下探必须显式传入 `--allow-descend`。
- 夹爪开合必须同时显式传入 `--use-gripper`。
- `block_grasp.yaml` 增加 `grasp/min_flange_z_m` 作为法兰高度硬限制。
- `grasp_z_m` 已提高为保守值，后续需按实际 TCP 和方块高度重新标定。

### 2026-07-20 Gripper TCP Draft TF

当前 URDF 已根据整机 STEP 更新夹爪草案 TF：

```text
flange -> gripper_base -> gripper_tcp
```

`carm_a3_motion` 默认会把 SDK 夹爪开口映射到 `/joint_states` 里的 `joint7/joint8`，用于 `robot_state_publisher` 发布夹爪 TF。默认映射为 `joint7 = joint8 = gripper_pos * 0.5`，并限幅到 URDF 里的 `0.037 m` 行程。

整机 STEP 推导的候选值记录在：

```text
docs/vendor/a3_full_model_step_probe.md
```
...
注意：当前 `flange -> gripper_base` 来自整机 STEP 装配坐标推断，`gripper_base -> gripper_tcp` 的 `0.149 m` 偏移仍来自夹爪前端几何估计，不等价于真实夹持接触点。正式把抓取规划从 `flange` 改成 `gripper_tcp` 前，需要用尺量或低速轻触桌面标定真实 TCP/contact height。当前下探抓取仍应保持 `grasp_z_m` 保守，并优先用 plan 检查。
